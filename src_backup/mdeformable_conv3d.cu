...
__device__ scalar_t modulated_deform_conv3d_im2col_trilinear(const scalar_t *bottom_data, const int data_width,const int data_length,
        ...
        scalar_t *data_col)
{
  ...
  CUDA_KERNEL_LOOP(index, n)
  {
    ...
    scalar_t *data_col_ptr = data_col+(c_col*batch_size + b_col) *height_col*width_col*length_col+
    		h_col*width_col*length_col+w_col*length_col+l_col;
    ...
    const scalar_t *data_mask_ptr = data_mask + (b_col * deformable_group + deformable_group_index)
    ...
    const int deformable_group, at::Tensor data_col)
{
  ...
  const int num_kernels = channels * height_col * width_col * length_col;
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      data_im.scalar_type(), "modulated_deform_conv3d_im2col_cuda", ([&] {
        ...
        modulated_deform_conv3d_im2col_gpu_kernel<<<GET_BLOCKS(num_kernels), CUDA_NUM_THREADS>>>(
            ...
            channels, deformable_group, height_col, width_col, length_col,data_col_);
      }));
...
int modulated_deform_conv3d_forward_cuda(
  ...
  at::Tensor columns =
  at::zeros({channels * kernel_h * kernel_w* kernel_l, height_out * width_out*length_out},
                input.options());
  ...
  for (int b = 0; b < batch; b++) {
	  ...
	  modulated_deform_conv3d_im2col_cuda(
        input[b], offset[b], mask[b],1, channels,
        height, width,length,
...
__global__ void modulated_deform_conv3d_gradient_gpu_kernel(
		const int n,const scalar_t *grad_col, const scalar_t *data_input, const scalar_t *data_offset, const scalar_t *data_mask,
		scalar_t * columns,
        ...
        scalar_t * grad_input,scalar_t *grad_offset, scalar_t *grad_mask)
{
  CUDA_KERNEL_LOOP(index, n)
  {
    ...
    int mask_hwl_ptr=offset_group_index*kernel_h*kernel_w*kernel_l*height_col*width_col*length_col+
...
void modulated_deform_conv3d_gradient_cuda(
    ...
    const at::Tensor data_input, const at::Tensor data_offset, const at::Tensor data_mask, at::Tensor columns,
    ...
    const int dilation_h, const int dilation_w, const int dilation_l,
    const int deformable_group, at::Tensor grad_input, at::Tensor grad_offset, at::Tensor grad_mask)
{
...
int modulated_deform_conv3d_backward_cuda(
  ...
  for (int b = 0; b < batch; b++) {
    ...
    modulated_deform_conv3d_gradient_cuda(
    		...
    		dilation_h, dilation_w, dilation_l,
    		deformable_group, grad_input[b],grad_offset[b],grad_mask[b]);
    columns = columns.view({group, columns.size(0) / group, columns.size(1)});
   // std::cout<<"columns \n"<<columns<<std::endl;
