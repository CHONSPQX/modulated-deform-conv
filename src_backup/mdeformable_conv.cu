...
__device__ scalar_t modulated_deform_conv2d_im2col_bilinear(const scalar_t *bottom_data, const int data_width,
        ...
        scalar_t *grad_im)
{
  CUDA_KERNEL_LOOP(index, n)
  {
    ...
    //CONDITION_PRINT(n-1,printf("\tmodulated_deformable_col2im_gpu_kernel index:%d c:%d i:%d j:%d\n",index,c,i,j));
    //CONDITION_PRINT(n-1,printf("\tmodulated_deformable_col2im_gpu_kernel b:%d h_out:%d w_out:%d h_in:%d w_in:%d \n",b,h_out,w_out,h_in,w_in));
    ...
    //CONDITION_PRINT(0,printf("\tmodulated_deformable_col2im_gpu_kernel cur_h:%d cur_w:%d cur_inv_h_data:%.2f cur_inv_w_data:%.2f \n",
    	//	cur_h,cur_w,cur_inv_h_data,cur_inv_w_data));
    for (int dy = -1; dy <= 1; dy++)
    {
      for (int dx = -1; dx <= 1; dx++)
      {
            ...
            abs(cur_inv_w_data - (cur_w + dx)) < 1)
        {
          ...
          //CONDITION_PRINT(0,printf("\tmodulated_deformable_col2im_gpu_kernel cur_bottom_grad_pos:%d \n",cur_bottom_grad_pos));
          //printf("\tmodulated_deformable_col2im_gpu_kernel index:%d cur_bottom_grad_pos:%d cur_h+dy:%d cur_w+dx:%d cur_inv_h_data:%.2f cur_inv_w_data:%.2f weight:%.2f \n",
           //   		index,cur_bottom_grad_pos,cur_h+dy,cur_w+dx,cur_inv_h_data,cur_inv_w_data,weight);
        }
      }
    }
  }
...
void modulated_deform_conv2d_col2im_cuda(
...
{
  //print_tensor_size("modulated_deformable_col2im_cuda---data_col size",data_col);//grad_output
  //print_tensor_size("modulated_deformable_col2im_cuda---data_offset size",data_offset);
  //print_tensor_size("modulated_deformable_col2im_cuda---data_mask size",data_mask);
  //print_tensor_size("modulated_deformable_col2im_cuda---grad_im size",grad_im);

  const int channel_per_deformable_group = channels / deformable_group;
  const int num_kernels = channels * kernel_h * kernel_w * batch_size * height_col * width_col;

  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      data_col.scalar_type(), "modulated_deform_conv2d_col2im_gpu_kernel", ([&] {
        const scalar_t *data_col_ = data_col.data<scalar_t>();
        const scalar_t *data_offset_ = data_offset.data<scalar_t>();
        const scalar_t *data_mask_ = data_mask.data<scalar_t>();
        scalar_t *grad_im_ = grad_im.data<scalar_t>();

        modulated_deform_conv2d_col2im_gpu_kernel<<<GET_BLOCKS(num_kernels), CUDA_NUM_THREADS>>>(
  ...
  if (err != cudaSuccess)
  {
    printf("error in modulated_deform_conv2d_col2im_cuda: %s\n", cudaGetErrorString(err));
  }
}
        ...
        scalar_t *grad_offset, scalar_t *grad_mask)
{
  CUDA_KERNEL_LOOP(index, n)
  {
    ...
    //CONDITION_PRINT(n-1,printf("\tmodulated_deformable_col2im_coord_gpu_kernel w:%d h:%d c:%d b:%d\n",w,h,c,b));
    //CONDITION_PRINT(n-1,printf("\tmodulated_deformable_col2im_coord_gpu_kernel index:%d offset_channels:%d deformable_group_index:%d channel_per_deformable_group:%d\n"
    //		,index,offset_channels,deformable_group_index,channel_per_deformable_group));
    for (int col_c = (offset_c / 2); col_c < channel_per_deformable_group; col_c += col_step)
    {
      const int col_pos = (((col_c * batch_size + b) * height_col) + h) * width_col + w;
      const int bp_dir = offset_c % 2;
  	  //CONDITION_PRINT(n-1,printf("\tmodulated_deformable_col2im_coord_gpu_kernel col_pos:%d %d %d\n",col_c * batch_size + b,h,w));
      ...
      {
        mval += data_col_ptr[col_pos] * modulated_deform_conv2d_im2col_bilinear(data_im_ptr + cnt * height * width, width, height, width, inv_h, inv_w);
      }
    ...
    //CONDITIONED_PRINT((((b * deformable_group + deformable_group_index) * kernel_h * kernel_w + offset_c / 2) * height_col + h) * width_col + w==0, printf("msg\n"));

  }
}

void modulated_deform_conv2d_col2im_coord_cuda(
    ...
    at::Tensor grad_offset, at::Tensor grad_mask)
{
  ...
  //print_tensor_size("modulated_deformable_col2im_coord_cuda---data_col size",data_col);
  //print_tensor_size("modulated_deformable_col2im_coord_cuda---data_im size",data_im);
  //print_tensor_size("modulated_deformable_col2im_coord_cuda---data_offset size",data_offset);
  //print_tensor_size("modulated_deformable_col2im_coord_cuda---data_mask size",data_mask);
  //print_tensor_size("modulated_deformable_col2im_coord_cuda---grad_offset size",grad_offset);
  //print_tensor_size("modulated_deformable_col2im_coord_cuda---grad_mask size",grad_mask);

  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      data_col.scalar_type(), "modulated_deform_conv2d_col2im_coord_gpu_kernel", ([&] {
        const scalar_t *data_col_ = data_col.data<scalar_t>();
        const scalar_t *data_im_ = data_im.data<scalar_t>();
        const scalar_t *data_offset_ = data_offset.data<scalar_t>();
        const scalar_t *data_mask_ = data_mask.data<scalar_t>();
        ...
        modulated_deform_conv2d_col2im_coord_gpu_kernel<<<GET_BLOCKS(num_kernels), CUDA_NUM_THREADS>>>(
  ...
  if (err != cudaSuccess)
  {
    printf("error in modulated_deform_conv2d_col2im_coord_cuda: %s\n", cudaGetErrorString(err));
  }
}
        ...
        scalar_t * grad_input,scalar_t *grad_offset, scalar_t *grad_mask)
{
  CUDA_KERNEL_LOOP(index, n)
  {
    ...
    int mask_hw_ptr=offset_group_index*kernel_h*kernel_w*height_col*width_col+k*height_col*width_col+hpos_col*width_col+wpos_col;
    ...
    columns[index]=col;
    //printf("index %d (hl,wl) %2d %2d (hh,wh) %2d %2d        v1 %f v2 %f v3 %f v4 %f gh %f gw %f\n",
    	//	index,h_low,w_low,h_high,w_high,(float)v1,(float)v2,(float)v3,(float)v4,
    		//(float)((-1*(1-dw)*v1-1*dw*v2+(1-dw)*v3+dw*v4)*dval),(float)((-1*(1-dh)*v1+(1-dh)*v2-1*dh*v3+dh*v4)*dval));
  }
}
...
int modulated_deform_conv2d_backward_cuda(
  ...
  for (int b = 0; b < batch; b++) {
    ...
    modulated_deform_conv2d_col2im_coord_cuda(
    		grad_columns, input[b], offset[b], mask[b], 1, channels, height, width,
        height_out, width_out, kernel_h, kernel_w, pad_h, pad_w, stride_h,
        stride_w, dilation_h, dilation_w, deformable_group, grad_offset[b],
        grad_mask[b]);// compute offset and mask gradient
    // gradient w.r.t. input data
    modulated_deform_conv2d_col2im_cuda(
    		grad_columns, offset[b], mask[b], 1, channels, height, width, height_out,
        width_out, kernel_h, kernel_w, pad_h, pad_w, stride_h, stride_w,
        dilation_h, dilation_w, deformable_group, grad_input[b]);// compute input gradient
    ...
    for (int g = 0; g < group; g++) {
      grad_weight[g] =
          grad_weight[g].flatten(1).addmm_(grad_output[b][g].flatten(1), columns[g].transpose(0, 1),1.0f,1.0f).view_as(grad_weight[g]);
      if (with_bias) {
...
int modulated_deform_conv2d_backward_cuda_v2(
  ...
  for (int b = 0; b < batch; b++) {
    ...
    modulated_deform_conv2d_gradient_cuda(
    		...
    		channels,height,width,height_out,width_out,kernel_h,kernel_w,pad_h,pad_w,stride_h,stride_w,dilation_h, dilation_w,deformable_group,
